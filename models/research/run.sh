...
dataset_dir=/data/HataFeng/csdn-ai-w8-data # 数据集目录，这里是写死的，记得修改
...
for i in {16..16}  # for循环中的代码执行5此，这里的左右边界都包含，也就是一共训练500个step，每100step验证一次
do
    echo "############" $i "runnning #################"
    last=$[$i*100]
    current=$[($i+1)*100]
...
# 在test.jpg上验证导出的模型
infe_dataset_dir = ./object_detection/data
